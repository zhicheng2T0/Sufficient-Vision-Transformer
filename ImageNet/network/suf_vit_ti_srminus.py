...
class suf_vit_srminus(nn.Module):
    ...
    def forward_features(self, x):
        ...
        for stage in range(len(self.change_layer_list)-1):
            x, cls_tokens, x_start,x_0= self.transformers[stage](x, cls_tokens,self.change_layer_list[stage])
        ...
        return cls_tokens,x_start_list,x_0_list
    ...
    def forward(self, x,allout=True):
        cls_token,x_start_list,x_0_list = self.forward_features(x)
        cls_token = self.head(cls_token[:, 0])
        if allout==True:
            return cls_token,x_start_list,x_0_list
        else:
            return cls_token
...
def suf_vit_ti_srminus(**kwargs):


    model = suf_vit_srminus(
        head_list=head_list,
