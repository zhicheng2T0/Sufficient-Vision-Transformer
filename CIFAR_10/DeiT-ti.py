...
model_name='aug2_vit_tiny'
#current_folder='/scratch/RDS-FEI-scr_2021_zhicheng-RW/cifar10_twins'
current_folder='.'
...
def main():
    ...
    model=model.cuda()
    #optimizer_network=torch.optim.AdamW(model.parameters(), lr=0.0005)
    ...
    #scheduler_cosine = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer_network, epochs-1)
    #scheduler = GradualWarmupScheduler(optimizer_network, multiplier=10, total_epoch=1, after_scheduler=scheduler_cosine)
    ...
    for epoch in range(epochs):
        for step, (batch_x,batch_y) in enumerate(train_loader):
            ...
            model.train()
            #print(batch_x.shape)
            batch_x=batch_x.cuda()#+torch.rand(batch_x.shape).cuda()
            #batch_y.to(DEVICE)
            batch_y=batch_y.cuda()
            #batch_x=reshape(batch_x,k)

            #batch_y_1h=reshape_y(batch_y)
            #batch_x, batch_y_s = mixup(batch_x, batch_y_1h, np.random.beta(0.2, 0.2))
            batch_x, batch_y_t = mixup_fn(batch_x, batch_y)

            prediction=model(batch_x)
            #batch_y_s=batch_y_s.cuda()
            #loss_net=loss_func(prediction,batch_y)
            ...
            if step==int(num_train/batch_size)-2:
            #if step%10==0:
                model.eval()
                #train result
                train_loss_sum=0
                #batch_y_1h=batch_y_1h.cuda()
                ...
                for stepv, (batch_xv,batch_yv) in enumerate(test_loader):
                    ...
                    #batch_xv=reshape(batch_xv,k)
                    batch_yv=batch_yv.cuda()
                    batch_yvt=batch_yv.cpu()
                    labelv_np=batch_yvt.detach().numpy()
                    #batch_yv1h=reshape_y(batch_yv)
                    val_count+=len(labelv_np)
                    prediction_v=model(batch_xv)#(512,10)
                    #batch_yv1h=batch_yv1h.cuda()
                ...
                with open(model_name+'_stdout'+".txt", "a") as std_out:
