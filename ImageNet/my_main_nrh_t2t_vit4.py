...
from network.nrh_t2t_vit4 import *
...
def train(train_loader, model, MINE_OG_list, MINE_IG_list, local_rank, rank, optimizer,optimizer_mine, criterion, epoch, base_lr,accumulate_num,batch_size):
    ...
    for i, (samples, targets) in enumerate(train_loader):
        ...
        prediction,x_ori,x_00,x_01,x_temp,x_10,x_11=current_output[0],current_output[1],current_output[2],current_output[3],current_output[4],current_output[5],current_output[6]
        ...
        torch.nn.utils.clip_grad_norm(parameters=model.parameters(), max_norm=5, norm_type=2.0)
...
def test(test_loader, model, local_rank, rank, validate_loss_fn):
    ...
    for i, (img, target) in enumerate(test_loader):
        ...
        # compute output
        output,x_ori,x_00,x_01,x_temp,x_10,x_11 = model(img)
...
def main():
    ...
    accumulate_num=1
    batch_size = 1024 // world_size
    num_workers = 8
    base_lr = 0.0005
    weight_decay = 0.05

    model = nhr_t2t_vit_tiny(init = 'trunc',norm_layer=partial(nn.LayerNorm, eps=1e-6))
    model = model.cuda()
