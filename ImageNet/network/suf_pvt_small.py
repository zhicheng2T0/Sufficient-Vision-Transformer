...
dim2=320
dim3=512
inter_layer_num_=[(num0,num1,200),
                (num1,num2,142),
                (num2,num3,36)]
...
class PyramidVisionTransformer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        for i in range(self.num_stages):
            ...
            counter=0
            for blk in block:
                if counter==len(block)-1 and i<self.num_stages - 1:
                    i_list.append(x)

                    garbage_tokens=self.fc_list[i](x)
                    g_list.append(self.block_list[i](x,garbage_tokens))
        ...
        return x[:, 0],i_list,o_list,g_list

    def forward(self, x):
        x ,i_list,o_list,g_list= self.forward_features(x)
        x = self.head(x)
