...
class Block2(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.drop_path(self.mlp(self.norm2(x_[:,0:x_.shape[1]//2+1,:])))
        x2 = self.drop_path(self.mlp2(self.norm2(x_[:,x_.shape[1]//2+1:,:])))
        return x[:,0:1,:],x[:,1:,:],x2
...
class sufvit(nn.Module):
    def __init__(self, head_list, token_num_list, token_dim_list, depth_list,
        ...
        self.flatten_dim=4*4*in_chans
        self.patch_to_embedding=nn.Linear(self.flatten_dim, token_dim_list[0])
        ...
        self.patch_size = patch_size
        self.pos_embed = nn.Parameter(data=get_sinusoid_encoding(n_position=token_num_list[0], d_hid=token_dim_list[0]), requires_grad=False)

        self.cls_token = nn.Parameter(
            torch.randn(1, 1, token_dim_list[0]),
            requires_grad=True
        )
...
def main():
    ...
    inter_layer_num_=[(num0,num1,num1),(num1,num2,num2)]
    ...
    for epoch in range(epochs):
        ...
        for step, (batch_x,batch_y) in enumerate(train_loader):
            ...
            loss_mine2=calculate_loss_mine(
                                        ...
                                        p_ig=m2_igp,
                                        q_ig=m2_igq)
            #loss_f1=loss_func(prediction,batch_y0)
